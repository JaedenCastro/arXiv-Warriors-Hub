import torch
# Used to be import torch.utils.data as data
import torch.utils.data as torch_data

# I THINK this used to be from data import process_data, not sure. I added a period to "data"
...
from transformers import AutoTokenizer, AutoConfig, AutoModelForSequenceClassification, AutoModel, TFAutoModel
...
def accuracy(out, y):
    # Columnwise mean accuracy for multilabel classification
    if len(y.shape) > 1:
...
def run_finetuning(args):
    ...
    if args.do_train:
        ...
        if hashed not in os.listdir() or args.retokenize_data:
            ...
            # Used to be data.process_data(text, labels, tokenizer, msl=args.msl)
        ...
        # Produce dataloaders
        # Used to use data, not torch_data (original)
        train_sampler = torch_data.RandomSampler(train_dataset)
        train_loader = torch_data.DataLoader(train_dataset, batch_size=args.batch_size, sampler=train_sampler)
        valid_loader = torch_data.DataLoader(valid_dataset, batch_size=args.batch_size, shuffle=False)
